...
| GET | `/<store-hash>.narinfo` | 不要 | narinfo（`Workers Cache → KV → R2 → 404`・404 は 60 秒 negative cache） |
...
## Workers Cache と purge

read 系の応答には `Cache-Tag`（`narinfo:<store-hash>` / `nar:<file-name>` / `cache-info`）が付く。edge のエントリは以下のタイミングで best-effort に purge される。

- `POST /api/gc/execute`: 削除した narinfo / NAR のタグを purge。レスポンスの `edge_purge_attempted` が成功タグ数（purge 非対応ランタイムでは 0）。
- `POST /api/publish/:build_id/finalize`: closure の `narinfo:<store-hash>` / `nar:<file-name>` タグを purge し、publish 直前に入った 404 negative cache（TTL 60 秒）を即時解消する。
