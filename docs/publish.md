...
| `CLOUDFLARE_ACCOUNT_ID` | 変数 | Cloudflare アカウント ID。 |
| `API_BASE_URL` | 変数 | デプロイ済み Worker の URL（例: `https://cf-edgenix.<account>.workers.dev`）。 |
| `R2_BUCKET_NAME` | 変数 | R2 バケット名（例: `cf-edgenix-nar`）。 |
| `KV_NAMESPACE_ID` | 変数 | KV 名前空間 ID。 |
| `HOST` | 変数 | publish 対象の nixosConfiguration 名（例: `myhost`）。 |
| `CACHE_DIR` | 変数 | `nix copy --to file://` の出力先ディレクトリ。 |

GitHub Actions での設定場所:
...
必要な Actions Secret / Variable（environment: production）:

| 名前 | 種別 | 内容 |
| --- | --- | --- |
| `CACHE_PRIVATE_KEY` | Secret | NAR 署名用秘密鍵 |
| `ADMIN_TOKEN` | Secret | Worker 管理 API トークン |
| `CLOUDFLARE_API_TOKEN` | Secret | R2 write / KV write 最小権限トークン |
| `CLOUDFLARE_ACCOUNT_ID` | Variable | CF アカウント ID |
| `API_BASE_URL` | Variable | Worker の URL |
| `R2_BUCKET_NAME` | Variable | R2 バケット名 |
| `KV_NAMESPACE_ID` | Variable | KV 名前空間 ID |

workflow は `push: branches: [main]` および `workflow_dispatch`（手動実行・host 入力）でトリガーされる。
